...
### Nitrogen
```{r}
summary(EC_Flow.Nutrients_Wide$Mixed_N)

# Evaluate assumption of normal distribution 
shapiro.test((EC_Flow.Nutrients_Wide$Mixed_N))
...
ggplot(EC_Flow.Nutrients_Wide, aes(x = Mixed_N)) +
  geom_histogram(binwidth = 20) 

### histogram shows data is not very rightly skewed 

qqnorm(EC_Flow.Nutrients_Wide$Mixed_N); qqline(EC_Flow.Nutrients_Wide$Mixed_N)
## some samples are higher than normal and some are lower

N.onesample <- t.test(EC_Flow.Nutrients_Wide$Mixed_N, mu = 10, alternative = "greater")
N.onesample

# Null hypothesis is that mean = 10, alternative is mean is less than 10
# p value >0.05 so we can not reject the null the mean =10

# plot 

N.plot <- ggplot(EC_Flow.Nutrients_Wide, aes(x = Mixed_N)) +
...
### Phosphorus
```{r}
summary(EC_Flow.Nutrients_Wide$TP)

# Evaluate assumption of normal distribution 
shapiro.test((EC_Flow.Nutrients_Wide$TP))

### shapiro wilke test the null hypothesis is that the data are a normal distribution 
### p value <0.05 shows we reject null and data is not normally distributed
######## "not well approximated by a normal distribution"

ggplot(EC_Flow.Nutrients_Wide, aes(x = TP)) +
  geom_histogram(binwidth = 1) 

### histogram shows data is not very rightly skewed 

qqnorm(EC_Flow.Nutrients_Wide$TP); qqline(EC_Flow.Nutrients_Wide$TP)
## samples are higher than normal distribution

P.onesample <- t.test(EC_Flow.Nutrients_Wide$TP, mu = 1, alternative = "greater")
P.onesample
...
P.plot <- ggplot(EC_Flow.Nutrients_Wide, aes(x = TP)) +
  #geom_density(stat = "count", fill = "gray") + #<-shows count so it's more jagged
  geom_density(fill = "gray") +
...
### Nitrogen
```{r}
shapiro.test(Club.Gorman_Flow.Nutrients_Wide$Mixed_N[Club.Gorman_Flow.Nutrients_Wide$Location == "Upstream"])
shapiro.test(Club.Gorman_Flow.Nutrients_Wide$Mixed_N[Club.Gorman_Flow.Nutrients_Wide$Location == "Downstream"])
var.test(Club.Gorman_Flow.Nutrients_Wide$Mixed_N ~ Club.Gorman_Flow.Nutrients_Wide$Location)
...
ggplot(Club.Gorman_Flow.Nutrients_Wide$Mixed_N, aes(x = Mixed_N, color = Location)) +
  geom_freqpoly()


N.twosample <- t.test(Club.Gorman_Flow.Nutrients_Wide$Mixed_N ~ Club.Gorman_Flow.Nutrients_Wide$Location)
N.twosample
...
### Phosphorus
```{r}
shapiro.test(Club.Gorman_Flow.Nutrients_Wide$TP[Club.Gorman_Flow.Nutrients_Wide$Location == "Upstream"])
shapiro.test(Club.Gorman_Flow.Nutrients_Wide$TP[Club.Gorman_Flow.Nutrients_Wide$Location == "Downstream"])
var.test(Club.Gorman_Flow.Nutrients_Wide$TP ~ Club.Gorman_Flow.Nutrients_Wide$Location)
...
ggplot(Club.Gorman_Flow.Nutrients_Wide, aes(x = TP, color = Location)) +
  geom_freqpoly()


P.twosample <- t.test(Club.Gorman_Flow.Nutrients_Wide$TP ~ Club.Gorman_Flow.Nutrients_Wide$Location)
P.twosample
...
### Nitrogen
```{r}
N.onesample.wilcox <- wilcox.test(Club.Gorman_Flow.Nutrients_Wide$Mixed_N, mu = 10, alternative = "greater")
N.onesample.wilcox
N.twosample.wilcox <- wilcox.test(Club.Gorman_Flow.Nutrients_Wide$Mixed_N ~ Club.Gorman_Flow.Nutrients_Wide$Location)
N.twosample.wilcox
```

### Phosphorus
```{r}
P.onesample.wilcox <- wilcox.test(Club.Gorman_Flow.Nutrients_Wide$TP, mu = 1, alternative = "greater")
P.onesample.wilcox
P.twosample.wilcox <- wilcox.test(Club.Gorman_Flow.Nutrients_Wide$TP ~ Club.Gorman_Flow.Nutrients_Wide$Location)
P.twosample.wilcox
```
...
###Nitrogen 
```{r}
NitrogenClub.regression <-lm(data = EC_Flow.Nutrients_Wide, Mixed_N ~ Discharge.C)
summary(NitrogenClub.regression)
## adj R squared says 1 % of variance is explained by depth 

cor.test(EC_Flow.Nutrients_Wide$Mixed_N, EC_Flow.Nutrients_Wide$Discharge.C)


NitrogenGorman.regression <-lm(data = EC_Flow.Nutrients_Wide, Mixed_N ~ Discharge.G)
summary(NitrogenGorman.regression)
## adj R squared says 1 % of variance is explained by depth 

cor.test(EC_Flow.Nutrients_Wide$Mixed_N, EC_Flow.Nutrients_Wide$Discharge.G)
```

###Phosphorus 
```{r}
PhosphorusClub.regression <-lm(data = EC_Flow.Nutrients_Wide, TP ~ Discharge.C)
summary(PhosphorusClub.regression)
## adj R squared says 8 % of variance is explained by depth 
## significant relationship p < 0.001

cor.test(EC_Flow.Nutrients_Wide$TP, EC_Flow.Nutrients_Wide$Discharge.C)


PhosphorusGorman.regression <-lm(data = EC_Flow.Nutrients_Wide, TP ~ Discharge.G)
summary(PhosphorusGorman.regression)
## adj R squared says 9 % of variance is explained by depth
## significant relationship p < 0.001

cor.test(EC_Flow.Nutrients_Wide$TP, EC_Flow.Nutrients_Wide$Discharge.G)
```
# Plot Regression
### Nitrogen
```{r}
Nitrogen_Discharge <- 
  ggplot(EC_Flow.Nutrients_Wide, aes (x=Discharge.G, y = Mixed_N))+
  geom_point()+
...
Nitrogen_logDischarge <- 
  ggplot(EC_Flow.Nutrients_Wide, aes (x=log(Discharge.G), y = Mixed_N))+
  geom_point()+
  geom_smooth(method=lm)+
  ylim(0,10)
...
logNitrogen_logDischarge <- 
  ggplot(EC_Flow.Nutrients_Wide, aes (x=log(Discharge.G), y = log(Mixed_N)))+
  geom_point()+
  geom_smooth(method=lm)+
  ylim(0,10)
...
Phosphorus_Discharge <- 
  ggplot(EC_Flow.Nutrients_Wide, aes (x=Discharge.G, y = TP))+
  geom_point()+
  geom_smooth(method=lm)+
  ylim(0,2.5)
...
Phosphorus_logDischarge <- 
  ggplot(EC_Flow.Nutrients_Wide, aes (x=log(Discharge.G), y = TP))+
  geom_point()+
  geom_smooth(method=lm)+
  ylim(0,10)
...
logPhosphorus_logDischarge <- 
  ggplot(EC_Flow.Nutrients_Wide, aes (x=log(Discharge.G), y = log(TP)))+
  geom_point()+
  geom_smooth(method=lm)
...
```{r}
Nitrogen_Time <- 
  ggplot(EC_Flow.Nutrients_Wide, aes (x=Date, y = Mixed_N))+
  geom_point()+
  geom_smooth(method=lm)
print(Nitrogen_Time)

 Phosphorus_Time <- 
  ggplot(EC_Flow.Nutrients_Wide, aes (x=Date, y = TP))+
  geom_point()+
  geom_smooth(method=lm)
...
### Nitrogen
```{r}
Nitrogen_Discharge2000 <- 
  ggplot(Post2000Nutrients , aes (x=log(Discharge.C), y = Mixed_N))+
  geom_point()+
  geom_smooth(method=lm)+
  ylim(0,10)
print(Nitrogen_Discharge2000)

Nitrogen_Time2000 <- 
  ggplot(Post2000Nutrients, aes (x=Date, y = Mixed_N))+
  geom_point()+
  geom_smooth(method=lm)
...
### Phosphorus 
```{r}
Phosphorus_Discharge2000 <- 
  ggplot(Post2000Nutrients, aes (x=log(Discharge.G), y = TP))+
  geom_point()+
  geom_smooth(method=lm)+
  ylim(0,2.5)
print(Phosphorus_Discharge2000)

Phosphorus_Time2000 <- 
  ggplot(Post2000Nutrients, aes (x=Date, y = TP))+
  geom_point()+
  geom_smooth(method=lm)
